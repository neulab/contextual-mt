...
class ContextualEnsembleModel(EnsembleModel):
    @torch.jit.export
    def forward_encoder(self, net_input: Dict[str, Tensor]):
        if not self.has_encoder():
            return None
        keys_to_remove = ["tgt_ctx_tokens", "tgt_ctx_lengths"]
        net_input = {k: v for k, v in net_input.items() if k not in keys_to_remove}
    ...
    @torch.jit.export
    def reorder_decoder_context(self, contex_tokens, new_order):
        """"""
        return contex_tokens.index_select(0, new_order)

    def has_incremental_states(self):
        # FIXME: there is some bug with incremental decoding
        return False
